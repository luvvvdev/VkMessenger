...
const typeingsDir = __dirname + "/../../app/types/vk"
const schemeDir = __dirname + "/schemas"

async function generate() {
  const [{ methods }, { definitions: responses }, { definitions: objects }, { errors }] =
    await Promise.all([
      readJSONFile(schemeDir + "/methods.json"),
      readJSONFile(schemeDir + "/responses.json"),
      readJSONFile(schemeDir + "/objects.json"),
      readJSONFile(schemeDir + "/errors.json"),
    ])
      ...
      result: TypesGenerator.promiseType(
        ts.factory.createQualifiedName(
          ...
          ts.factory.createIdentifier(
            toPascalCase(
              (method.responses.response || method.responses.keyResponse).$ref.replace(
