...
const jsonSchemaTypes = {
    ...
    type:
      "enum" in type
        ? TypesGenerator.union(
            type.enum.map((enumName) => ts.factory.createStringLiteral(String(enumName))),
          )
  ...
  integer: ({ type }) => ({
    ...
    type:
      "enum" in type
        ? TypesGenerator.union(
            type.enum.map((enumName) => ts.factory.createNumericLiteral(String(enumName))),
          )
...
function parseJSONSchema(schema, payload) {
  return Object.entries(schema).map(([key, value]) => parseJSONObject(key, value, payload))
}
