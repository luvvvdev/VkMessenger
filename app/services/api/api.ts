...
export class Api {
  ...
  async getHistory(
    ...
    try {
      const response: ApiResponse<any, any> = await this.apisauce.get(
  ...
  async sendMessage(
    ...
    try {
      const response: ApiResponse<VkResponse<MessagesSendResponse>, any> = await this.apisauce.post(
        `/method/messages.send`,
        {},
        {
          params: {
            random_id: random_id || Number(new Date().getMilliseconds()),
            peer_id,
            message,
          },
        },
  ...
  async getLongPollUpdates(
    server: string,
    key: string,
    ts: number,
  ): Promise<GetLongPollUpdatesResult> {
    const response: ApiResponse<any, any> = await this.apisauce.get(
      `https://${server}`,
      {
        act: "a_check",
  ...
  async getLongPollHistory(
    params: MessagesGetLongPollHistoryParams,
  ): Promise<GetLongPollHistoryResult> {
    const response: ApiResponse<any, any> = await this.apisauce.get(
  ...
  async getMessageByConversationId(peer_id: number, conversation_id: number) {
    try {
      const response: ApiResponse<
        VkResponse<MessagesGetByConversationMessageIdResponse>,
        any
      > = await this.apisauce.get(`/method/messages.getByConversationMessageId`, {
        extended: true,
  ...
  async getConversationsById(peer_ids: number[]): Promise<GetConversationsByIdResult> {
    try {
      const response: ApiResponse<
        VkResponse<MessagesGetConversationsByIdExtendedResponse>,
        any
      > = await this.apisauce.get(`/method/messages.getConversationsById`, {
        extended: true,
        peer_ids: peer_ids,
      })
