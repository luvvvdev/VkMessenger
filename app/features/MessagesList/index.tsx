...
export default ({ peer_id }: MessagesProps) => {
  ...
  const historyLoading = useSelector(
    (state: RootState) => state.loading.effects.history.get.loading,
  )
  const loadingMore = useSelector(
    (state: RootState) => state.loading.effects.history.loadMore.loading,
  )

  const messages = useSelector<RootState, Array<MessagesMessage[]>>((state) => {
    const messages = state.history.items[peer_id]?.items

    const groupedMessages: { [key: string]: MessagesMessage[] } = _.groupBy<
      MessagesMessage[],
      string
    >(messages, (item) => startOfDay(item.date * 1000).toUTCString())
  ...
  useEffect(() => {
    if (messages.length === 0) {
      dispatch.history.get({ peer_id, offset: 0, count: 200 }).catch((error) => {
        console.log("error")
  ...
  const onEndReached = ({ distanceFromEnd }) => {
    ...
    dispatch.history.loadMore({ count: 200 }).catch((e) => {
