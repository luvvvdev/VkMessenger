...
const history = createModel<RootModel>()({
  ...
  reducers: {
    ...
    push: (state, payload: { peer_id: number; history: any }) => ({
      ...state,
      current_id: payload.peer_id,
      items: { ...state.items, [payload.peer_id]: payload.history },
    }),
    ...
    setCurrentId: (state, payload: { peer_id: number | null }) => ({
      ...state,
      current_id: payload.peer_id,
    }),
    addMessage: (state, { message }: { message: MessagesMessage }) => {
      ...
      return {
        ...
        items: {
          [message.peer_id]: {
            ...state.items[message.peer_id],
            count: state.items[message.peer_id].count + 1,
            items: [message, ...state.items[message.peer_id].items],
          },
        },
      }
    },
    setMessageLoading: (
      state,
      { peer_id, msg_id, rid }: { peer_id: number; msg_id: number; rid: number },
    ) => {
      ...
      return {
        ...state,
        items: { ...state.items, [peer_id]: { ...targetHistory, items: targetHistoryMessages } },
      }
    },
  },
  effects: (dispatch) => ({
    get: async (payload: HistoryGetEffect, state) => {
      ...
      try {
        ...
        const startMessageId =
          neededHistory &&
          neededHistory.items.length > 0 &&
          (!payload.offset || payload.offset === 0)
            ? neededHistory.items[neededHistory.items.length - 1].id
            : undefined
        ...
        dispatch.history.push({
          ...
          history: {
            ...
            items: [
              ...(payload.offset > 0 || (neededHistory && neededHistory.items.length > 0)
                ? data.items.length > 0
                  ? [...neededHistory.items, ...data.items]
                  : neededHistory.items
                : data.items),
            ],
          },
    ...
    loadMore: async (payload: { count?: number }, state) => {
      ...
      await dispatch.history.get({
        offset,
        count: 200,
        peer_id: state.history.current_id,
      } as HistoryGetEffect)
    },
