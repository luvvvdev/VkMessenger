...
export const conversations = createModel<RootModel>()({
  ...
  effects: (dispatch) => ({
    ...
    editLastMessage: async (
      { message, retries = 0 }: { message: MessagesMessage; retries?: number },
      state,
    ) => {
      try {
        console.log("updating last message")

        const currentConversationPredicate = (value: MessagesConversationWithMessage) =>
          value.last_message.peer_id === message.peer_id
        let currentConversationIndex = _.findIndex(
          state.conversations.items,
          currentConversationPredicate,
        )
        let currentConversation = state.conversations.items[
          currentConversationIndex
        ] as MessagesConversationWithMessage
        ...
        if (!currentConversation) {
          ...
          currentConversation = {
            ...
            conversation: newData.data.response.items[0],
          }

          dispatch.conversations.update({
            ...state.conversations,
            items: [...state.conversations.items, currentConversation],
            profiles: [...state.conversations.profiles!, ...newData.data.response.profiles[0]],
            groups: [...state.conversations.groups!, ...newData.data.response.groups[0]],
          })
        } else {
          console.log("update last message")
          ;(currentConversation as MessagesConversationWithMessage).last_message =
